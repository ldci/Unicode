...
;--to-char is limited to 65535 (#000000000000FFFF) integer value
...
getCodes: func [
	...
	s: codes/:codesIndex
	ss: split s " "
	intRange: to-block split second ss "-"
	starting: to integer! intRange/1 
	ending: to integer!  intRange/2
	range: ending - starting + 1
	if starting = 0 [range: -- range]
	n: length? ss 
	label: third ss
	if n > 3 [
		i: 3 
		clear label
		while [i <= n][
			append  label  rejoin [ss/:i " "] ++ i] 
	]
	i: starting
	while [i <= ending] [
		print rejoin ["Unicode " form i ": " form to char! i]
		i: i + 1
	]
	print [rejoin [s " (" range " symbols)"]]
]

;--sample 
t: dt [getCodes 32];--88 max 
print ["In" third t * 1000 "ms"]
